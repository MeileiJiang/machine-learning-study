...
kern_smooth = function(h, X =X1, Y = Y1, Tick = time, Time = Toydata$Time){
  ...
  for(k in c(1:101)){
    ...
    beta12[k] = coef(wfit)["X2",]
    beta13[k] = coef(wfit)["X3",]
  }
...
pdf(file = "figures/kernel_smooth_glmnet.pdf", height = 10, width = 8)
grid.arrange(g1.1, g1.2, g1.3, g1.4, g1.5, g1.6, ncol = 2 )
dev.off()

pdf(file = "figures/kernel_smooth_glmmet2.pdf", height = 12, width = 10)
