function JIVEPreVisualQF(datablock, rank, numcompshow, dataname) 
% Visualization of JIVE Step 1 rank selection
...
for ib = 1:k;
    ...
    for ir = 1:length(rt);
        ...
        if icolorindex == 0;
            line(XL, [s(rt(ir)) s(rt(ir))], 'linewidth', 1, 'linestyle', '--', 'color', colormat(7, :))
        else
            line(XL, [s(rt(ir)) s(rt(ir))], 'linewidth', 1, 'linestyle', '--', 'color', colormat(icolorindex, :))
        end
    end
    subplot(1, 2, 2);
    ...
    savestr = [dataname{ib} 'JIVEStep1ScreePlot'];
    axes('Units','Normal');
    ...
    for ir = 1:length(rt);
        icolorindex = mod(ir, 7);
        if icolorindex == 0;
            line(XL, [log10(s(rt(ir))) log10(s(rt(ir)))], 'linewidth', 1, 'linestyle', '--', 'color', colormat(7, :))
        else
            line(XL, [log10(s(rt(ir))) log10(s(rt(ir)))], 'linewidth', 1, 'linestyle', '--', 'color', colormat(icolorindex, :))
        end
    end;
    ...
    savestr = [dataname{ib} 'JIVEStep1LogScreePlot'];
    axes('Units','Normal');
